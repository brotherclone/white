...
MODULE_NAME = "white_ideation.agents.states.orange_agent_state"
CLASS_NAME = "OrangeAgentState"
...
def test_orange_agent_state_is_callable():
...
def test_orange_agent_state_has_required_fields():
    """Test that OrangeAgentState has all required fields for the orange workflow."""
    cls = _get_class_or_skip()
    try:
        obj = cls()
    except Exception:
    ...
    # Check for orange agent specific fields
    expected_fields = [
        "synthesized_story",
        "search_results",
        "corpus_stories",
    ...
    for field in expected_fields:
        assert hasattr(obj, field), f"OrangeAgentState should have field '{field}'"


def test_orange_agent_state_field_defaults():
    """Test that OrangeAgentState fields have correct defaults."""
    cls = _get_class_or_skip()
    try:
        state = cls()
    except Exception:
    ...
    # Test None defaults
    assert state.synthesized_story is None
    assert state.search_results is None
    assert state.corpus_stories is None
    assert state.selected_story_id is None
    assert state.symbolic_object is None
    assert state.gonzo_perspective is None
    assert state.mythologized_story is None

...
MODULE_NAME = "white_ideation.agents.states.red_agent_state"
CLASS_NAME = "RedAgentState"
...
def test_red_agent_state_is_callable():
...
def test_red_agent_defaults():
    """Test that RedAgentState inherits from BaseRainbowAgentState."""
...
def test_red_agent_state_has_required_fields():
    """Test that RedAgentState has all required fields for the red workflow."""
    cls = _get_class_or_skip()
    try:
        obj = cls()
    except Exception:
    ...
    # Check for red agent specific fields
    expected_fields = [
        "main_generated_book",
        "current_reaction_book",
        "should_respond_with_reaction_book",
        "should_create_book",
        "reaction_level",
    ]

    for field in expected_fields:
        assert hasattr(obj, field), f"RedAgentState should have field '{field}'"


def test_red_agent_state_field_defaults():
    """Test that RedAgentState fields have correct defaults."""
    cls = _get_class_or_skip()
    try:
        state = cls()
    except Exception:
    ...
    # Test None defaults
    assert state.main_generated_book is None

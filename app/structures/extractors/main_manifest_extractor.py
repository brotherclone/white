...
class ManifestExtractor:
    ...
    def __init__(
        ...
        load_manifest_on_init: bool = True,
        *args,
        **kwargs,
    ) -> None:
        """
    ...
    @staticmethod
    def load_manifest(m_id: str) -> Manifest | None:
        """
        ...
        """
        try:
            return manifest_loader.load_manifest(m_id)
        except ValueError as err:
            print(f"Error loading manifest {manifest_id}: {err}")
            return None
    ...
    def _calculate_boundary_fluidity_score(segment: Dict[str, Any]) -> float:
        ...
        lyric_bleeding = len(
            [
                ...
                if lyric.get("temporal_relationship")
                not in [TemporalRelationship.CONTAINED, TemporalRelationship.MATCH]
            ]
        )
    ...
    def load_audio_segment(
        ...
        try:
            return self.audio_extractor.extract_segment_features(
    ...
    def load_midi_segment(
        self, midi_path: str, start_time: float, end_time: float
    ) -> Dict[str, Any]:
        """Load MIDI features for a specific time segment"""
        try:
            return self.midi_extractor.extract_segment_features(
                midi_path, start_time, end_time
            )
    ...
    def generate_multimodal_segments(
        self,
        mms_yaml_path: str,
        mms_lrc_path: str = None,
        mms_audio_path: str = None,
        mms_midi_path: str = None,
    ) -> pd.DataFrame:
        """
        ...
        if mms_audio_path:
            print(
                "⚠️  WARNING: mms_audio_path parameter is deprecated. Audio tracks are now loaded from manifest."
            )

        print("=== MULTIMODAL REBRACKETING TRAINING DATA GENERATION ===")
        ...
        for section in manifest.structure:
            ...
            for lyric in intersecting_lyrics:
                ...
                confidence = (
                    overlap_duration / lyric_duration if lyric_duration > 0 else 0
                )
            ...
            if mms_audio_path and Path(mms_audio_path).exists():
                segment["audio_features"] = self.load_audio_segment(
                    mms_audio_path, section_start, section_end
                )
            ...
            if mms_midi_path and Path(mms_midi_path).exists():
                segment["midi_features"] = self.load_midi_segment(
                    mms_midi_path, section_start, section_end
                )
            ...
            if hasattr(manifest, "audio_tracks") and manifest.audio_tracks:
                ...
                for track in manifest.audio_tracks:
                    ...
                    if os.path.exists(audio_path):
                        try:
                            audio_features = self.load_audio_segment(
                                audio_path, section_start, section_end
                            )
                            track_data = {
                            ...
                            print(
                                f"Warning: Could not load track {getattr(track, 'id', None)} ({getattr(track, 'audio_file', None)}): {e}"
                            )
                    else:
                        print(f"Warning: Audio file not found: {audio_path}")

            # Calculate boundary fluidity metrics across all modalities
            segment["rebracketing_score"] = self._calculate_boundary_fluidity_score(
                segment
            )
        ...
        dfrm["has_temporal_bleeding"] = dfrm["lyrical_content"].apply(
            lambda x: any(
                lyric["temporal_relationship"] not in ["contained", "exact_match"]
                for lyric in x
            )
        )
        ...
        if mms_audio_path:
            dfrm["audio_energy"] = dfrm["audio_features"].apply(
                lambda x: x.get("rms_energy", 0)
            )
            dfrm["spectral_complexity"] = dfrm["audio_features"].apply(
                lambda x: x.get("spectral_centroid", 0)
            )

        if mms_midi_path:
            dfrm["midi_density"] = dfrm["midi_features"].apply(
                lambda x: x.get("note_density", 0)
            )
            dfrm["pitch_complexity"] = dfrm["midi_features"].apply(
                lambda x: x.get("pitch_variety", 0)
            )
        ...
        if mms_midi_path:
            print(
                f"Average MIDI note density: {dfrm['midi_density'].mean():.2f} notes/second"
            )

        return dfrm

    def generate_enhanced_multimodal_segments(
        self,
        mm_yaml_path: str,
        mm_lrc_path: str = None,
        mm_audio_path: str = None,
        mm_midi_path: str = None,
    ) -> pd.DataFrame:
        """Generate multimodal training dataset with rebracketing methodology analysis"""
        print("=== ENHANCED MULTIMODAL REBRACKETING TRAINING DATA GENERATION ===")
        ...
        rebracketing_features = (
            concept_extractor.generate_rebracketing_training_features()
        )
        concept_analysis = concept_extractor.concept_analysis

        print(
            f"Analyzed concept field - Rebracketing Type: {concept_analysis.rebracketing_type}"
        )
        print(
            f"Memory Discrepancy: {concept_analysis.original_memory} → {concept_analysis.corrected_memory}"
        )
        ...
        for section in manifest["structure"]:
            ...
            if lyrics:
                for lyric in lyrics:
                    ...
                    if not (lyric_end <= section_start or lyric_start >= section_end):
                        ...
                        lyric_duration = lyric_end - lyric_start
                        confidence = (
                            overlap_duration / lyric_duration
                            if lyric_duration > 0
                            else 0
                        )
                        ...
                        lyric_data = {
                            ...
                            "contains_memory_reference": self._check_memory_references(
                                lyric["text"]
                            ),
                            "contains_temporal_markers": self._check_temporal_markers(
                                lyric["text"]
                            ),
                            "contains_rebracketing_language": self._check_rebracketing_language(
                                lyric["text"]
                            ),
                            "lyrical_boundary_fluidity": self._calculate_lyrical_boundary_fluidity(
                                temporal_rel, confidence
                            ),
                        }
            ...
            segment = {
                ...
                "concept_analysis": (
                    concept_analysis.__dict__ if concept_analysis else {}
                ),
                "ontological_category": (
                    concept_analysis.ontological_category if concept_analysis else None
                ),
                "memory_discrepancy_severity": (
                    concept_analysis.memory_discrepancy_severity
                    if concept_analysis
                    else 0.0
                ),
                "temporal_complexity": rebracketing_features.get(
                    "temporal_rebracketing_complexity", 0.0
                ),
            ...
            if hasattr(manifest, "audio_tracks") and manifest.audio_tracks:
                        ...
                        try:
                            ...
                            track_data = {
                                ...
                                **audio_features,
                                ...
                                "spectral_instability": self._calculate_spectral_instability(
                                    audio_features
                                ),
                                "temporal_discontinuity": self._calculate_temporal_discontinuity(
                                    audio_features
                                ),
                                "genre_shift_indicators": self._detect_genre_shift_indicators(
                            ...
                            segment["audio_tracks_features"].append(track_data)
                            print(
                                f"  ✅ Track {track.id}: {track.description} - {track_data['player']}"
                            )

                        except Exception as e:
                            print(
                                f"  ⚠️  Could not load track {track.id} ({track.audio_file}): {e}"
                            )
                    else:
                ...
                if segment["audio_tracks_features"]:
                    segment["audio_rebracketing_metrics"] = {
                        ...
                        "player_diversity": len(
                            set([t["player"] for t in segment["audio_tracks_features"]])
                        ),
                    }
                    print(
                        f"  ✅ Added aggregate audio rebracketing metrics for {section['section_name']}"
                    )
            else:
                segment["audio_tracks_features"] = []
                segment["audio_rebracketing_metrics"] = {}
                print("  ℹ️  No audio tracks in manifest")

            # Add MIDI features with rebracketing analysis
            if mm_midi_path and Path(mm_midi_path).exists():
                midi_features = self.load_midi_segment(
                    mm_midi_path, section_start, section_end
                )
                segment["midi_features"] = midi_features
                segment["midi_rebracketing_metrics"] = {
                    "harmonic_discontinuity": self._calculate_harmonic_discontinuity(
                        midi_features
                    ),
                    "rhythmic_rebracketing": self._calculate_rhythmic_rebracketing(
                        midi_features
                    ),
                    "instrumental_genre_mixing": self._detect_instrumental_genre_mixing(
                        midi_features
                    ),
                }
                print(
                    f"  ✅ Added enhanced MIDI features for {section['section_name']}"
                )

            # Calculate comprehensive boundary fluidity across all modalities
            segment["comprehensive_rebracketing_score"] = (
                self._calculate_comprehensive_rebracketing_score(segment)
            )
        ...
        df["contains_memory_references"] = df["lyrical_content"].apply(
            lambda x: any(lyric.get("contains_memory_reference", False) for lyric in x)
        )
        ...
        if len(df) > 0 and df["audio_track_count"].sum() > 0:
            df["avg_audio_energy"] = df["audio_tracks_features"].apply(
                lambda tracks: (
                    np.mean([t.get("rms_energy", 0) for t in tracks]) if tracks else 0
                )
            )
            df["avg_spectral_complexity"] = df["audio_tracks_features"].apply(
                lambda tracks: (
                    np.mean([t.get("spectral_centroid", 0) for t in tracks])
                    if tracks
                    else 0
                )
            )
            df["audio_rebracketing_intensity"] = df["audio_rebracketing_metrics"].apply(
                lambda x: (
                    (
                        x.get("avg_spectral_instability", 0)
                        + x.get("avg_temporal_discontinuity", 0)
                    )
                    / 2
                    if x
                    else 0
        ...
        # MIDI metrics
        if mm_midi_path:
            df["midi_density"] = df["midi_features"].apply(
                lambda x: x.get("note_density", 0)
            )
            df["pitch_complexity"] = df["midi_features"].apply(
                lambda x: x.get("pitch_variety", 0)
            )
            df["harmonic_rebracketing_intensity"] = df[
                "midi_rebracketing_metrics"
            ].apply(lambda x: x.get("harmonic_discontinuity", 0))

        print("\n=== ENHANCED REBRACKETING ANALYSIS ===")
        print(f"Total segments: {len(df)}")
        print(
            f"Segments following Orange methodology: {df['follows_orange_methodology'].sum()}"
        )
        print(
            f"Segments with memory boundary crossing: {df['memory_boundary_crossing'].sum()}"
        )
        print(f"Segments with temporal bleeding: {df['has_temporal_bleeding'].sum()}")
        print(
            f"Average rebracketing score: {df['comprehensive_rebracketing_score'].mean():.4f}"
        )
        ...
        print(
            f"Unique players: {set([p for players in df['players'] for p in players])}"
        )

        if len(df) > 0 and df["audio_track_count"].sum() > 0:
            print(
                f"Average audio rebracketing intensity: {df['audio_rebracketing_intensity'].mean():.4f}"
            )
        if mm_midi_path:
            print(
                f"Average harmonic rebracketing intensity: {df['harmonic_rebracketing_intensity'].mean():.4f}"
            )
    ...
    @staticmethod
    def _calculate_lyrical_boundary_fluidity(
        temporal_rel: str, confidence: float
    ) -> float:
    ...
    @staticmethod
    def _calculate_section_rebracketing_score(
        section: dict, lyrics: list, concept_analysis
    ) -> float:
        ...
        if lyrics:
            avg_fluidity = sum(
                lyric.get("lyrical_boundary_fluidity", 0) for lyric in lyrics
            ) / len(lyrics)
    ...
    def _identify_boundary_crossing_indicators(section: dict, lyrics: list) -> list:
        ...
        if lyrics:
            memory_refs = sum(
                1 for lyric in lyrics if lyric.get("contains_memory_reference", False)
            )
            if memory_refs > 0:
                indicators.append("memory_reference_clustering")

            temporal_refs = sum(
                1 for lyric in lyrics if lyric.get("contains_temporal_markers", False)
            )
            if temporal_refs > 1:
    ...
    def _calculate_comprehensive_rebracketing_score(segment: Dict[str, Any]) -> float:
        ...
        if segment.get("lyrical_content"):
            lyric_scores = [
                lyric.get("lyrical_boundary_fluidity", 0)
                for lyric in segment["lyrical_content"]
            ]
            if lyric_scores:
        ...
        if segment.get("audio_rebracketing_metrics"):
            ...
            audio_score = (
                audio_metrics.get("avg_spectral_instability", 0)
                + audio_metrics.get("avg_temporal_discontinuity", 0)
            ) / 2
    ...
    def _detect_genre_shift_indicators(
        self, audio_features: Dict[str, Any], section: Dict
    ) -> List[str]:
    ...
    def _detect_instrumental_genre_mixing(
        self, midi_features: Dict[str, Any]
    ) -> List[str]:
    ...
    def save_training_data(
        self, df: pd.DataFrame, base_path: str, format: str = "parquet"
    ) -> Dict[str, str]:
        """
        ...
        if format in ["hybrid", "all"]:
            ...
            metadata = {
                ...
                "segment_types": (
                    df["segment_type"].unique().tolist()
                    if "segment_type" in df.columns
                    else []
                ),
                "has_audio": (
                    int(df["audio_features"].notna().sum())
                    if "audio_features" in df.columns
                    else 0
                ),
                "has_midi": (
                    int(df["midi_features"].notna().sum())
                    if "midi_features" in df.columns
                    else 0
                ),
                "has_lyrics": (
                    int(df["lyric_count"].sum()) if "lyric_count" in df.columns else 0
                ),
            }
    ...
    def load_training_data(
        self, base_path: str, format: str = "parquet"
    ) -> pd.DataFrame:
        """
    ...
    def _flatten_for_parquet(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_flat.columns:
            if df_flat[col].dtype == "object":
                sample_val = (
                    df_flat[col].dropna().iloc[0]
                    if len(df_flat[col].dropna()) > 0
                    else None
                )
                if (
                    sample_val is not None
                    and hasattr(sample_val, "__class__")
                    and hasattr(sample_val.__class__, "__module__")
                ):
                    module_name = sample_val.__class__.__module__
                    if module_name and (
                        "music21" in module_name
                        or "pretty_midi" in module_name
                        or "app.structures" in module_name
                    ):
                        df_flat[col] = df_flat[col].apply(
                            lambda x: str(x) if x is not None else None
                        )
        ...
        if "audio_tracks_features" in df_flat.columns:
            ...
            df_flat["players_csv"] = df_flat["audio_tracks_features"].apply(
                lambda tracks: (
                    ",".join(sorted(set([t["player"] for t in tracks])))
                    if tracks
                    else ""
                )
            )
            df_flat["avg_rms_energy"] = df_flat["audio_tracks_features"].apply(
                lambda tracks: (
                    float(np.mean([t.get("rms_energy", 0) for t in tracks]))
                    if tracks
                    else 0.0
                )
            )
            df_flat["avg_spectral_centroid"] = df_flat["audio_tracks_features"].apply(
                lambda tracks: (
                    float(np.mean([t.get("spectral_centroid", 0) for t in tracks]))
                    if tracks
                    else 0.0
                )
            )
        ...
        if "audio_rebracketing_metrics" in df_flat.columns:
            df_flat["audio_rebrack_avg_spectral_instability"] = df_flat[
                "audio_rebracketing_metrics"
            ].apply(
                lambda x: (
                    x.get("avg_spectral_instability", 0) if isinstance(x, dict) else 0
                )
            )
            df_flat["audio_rebrack_avg_temporal_discontinuity"] = df_flat[
                "audio_rebracketing_metrics"
            ].apply(
                lambda x: (
                    x.get("avg_temporal_discontinuity", 0) if isinstance(x, dict) else 0
                )
            )
            df_flat["audio_rebrack_player_diversity"] = df_flat[
                "audio_rebracketing_metrics"
            ].apply(
    ...
    def _unflatten_from_parquet(self, df_flat: pd.DataFrame) -> pd.DataFrame:
            ...
            def safe_json_loads(x):
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            "Warning: Could not parse as literal either, returning empty list"
                        )
                        return []
        ...
        midi_cols = [
            ...
            if col.startswith("midi_") and col != "midi_features"
        ]
        if midi_cols:
            df["midi_features"] = df.apply(
                lambda row: {col.replace("midi_", ""): row[col] for col in midi_cols},
                axis=1,
            )
        ...
        audio_rebrack_cols = [
            col for col in df.columns if col.startswith("audio_rebrack_")
        ]
        if audio_rebrack_cols:
            df["audio_rebracketing_metrics"] = df.apply(
                lambda row: {
                    col.replace("audio_rebrack_", ""): row[col]
                    for col in audio_rebrack_cols
                },
                axis=1,
            )
        ...
        if "lyrical_content_json" in df.columns:
            ...
            df["lyrical_content"] = df["lyrical_content_json"].apply(
                safe_json_loads_lyrics
            )
        ...
        if "players_csv" in df.columns and "players" not in df.columns:
            df["players"] = df["players_csv"].apply(lambda x: x.split(",") if x else [])
        ...
        json_columns = [
            ...
            and col not in ["audio_tracks_json", "lyrical_content_json"]
        ]
        for json_col in json_columns:
            ...
            def safe_parse(x):
                ...
                try:
                    ...
                    print(f"Warning: Could not parse JSON for {original_col}: {err}")
                    try:
                        import ast
...
if __name__ == "__main__":
    ...
    print(
        "Loaded manifest:", extractor.manifest.title if extractor.manifest else "None"
    )

    # Example: generate_multimodal_segments (if files exist)
    if os.path.exists(yaml_path):
        try:
            df = extractor.generate_multimodal_segments(
                yaml_path, lrc_path, None, midi_path
            )
            ...
            print("\n=== LOADING TEST ===")
            df_loaded_parquet = extractor.load_training_data(
                base_save_path, format="parquet"
            )
            df_loaded_pickle = extractor.load_training_data(
                base_save_path, format="pickle"
            )

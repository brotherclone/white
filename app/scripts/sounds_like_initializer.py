...
from app.scripts.reference_plan_helper import enrich_sounds_like
...
async def initialize_sounds_like(artist_list):
    for artist_name in artist_list:
        ...
        try:
            ...
            if a_rainbow_artist:
                print(f"Enriched sounds like for {a_rainbow_artist.name}: {a_rainbow_artist.profile if a_rainbow_artist.profile else 'No profile available'}")
            else:
...
if __name__ == "__main__":
    async def main():
        sounds = await initialize_sounds_like(NEGATIVE_ARTISTS)

...
Usage:
    # Single song
    python -m app.generators.midi.production.song_evaluator <production_dir>

    # Cross-song comparison (rank multiple songs)
    python -m app.generators.midi.production.song_evaluator --compare <dir1> <dir2> ...

    # Optional: re-score promoted MIDIs with Refractor
    python -m app.generators.midi.production.song_evaluator <production_dir> --rescore
"""
...
def _rescore_phases(
    ...
    scorer_path = (
        Path(__file__).parent.parent.parent.parent.parent / "training" / "refractor.py"
    )
    onnx_path = (
        Path(__file__).parent.parent.parent.parent.parent
        / "training"

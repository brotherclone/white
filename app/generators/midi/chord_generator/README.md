...
The chord generation engine used by `chord_pipeline.py`. Transforms a corpus of MIDI chord progressions into a queryable database (Polars/Parquet) with Markov transition graphs (NetworkX), then generates and scores candidate progressions via brute-force sampling.

## Overview

The corpus parses **2,746 MIDI files** containing:
...
```
app/generators/midi/chord_generator/
├── __init__.py
...
See `app/generators/midi/chord_pipeline.py` and `app/generators/midi/README.md` for full documentation.

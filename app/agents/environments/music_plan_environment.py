...
REFERENCE_PLANS_DIR = "/Volumes/LucidNonsense/White/app/plans/reference"
...
class MusicPlanEnvironment(gym.Env):
    ...
    def _load_reference_plans(reference_plans_dir: str) -> dict or None:
        ...
        if os.path.exists(reviewed_dir):
            for filename in os.listdir(reviewed_dir):
                if filename.endswith(".yml"):
                    yml_path = os.path.join(reviewed_dir, filename)
            ...
            print(f"Reference plans directory '{reviewed_dir}' does not exist.")
            return None

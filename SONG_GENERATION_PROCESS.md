...
```bash
python -m app.generators.midi.chord_pipeline --production-dir <dir> --song-proposal <yml>
```
...
- Promote: `python -m app.generators.midi.promote_part --production-dir <dir> --phase chords`
...
```bash
python -m app.generators.midi.drum_pipeline --production-dir <dir>
```
...
```bash
python -m app.generators.midi.bass_pipeline --production-dir <dir>
```
...
```bash
.venv312/bin/python -m app.generators.midi.melody_pipeline --production-dir <dir> --singer <name>
```
...
```bash
.venv312/bin/python -m app.generators.midi.composition_proposal --production-dir <dir> --song-proposal <yml>
```
...
```bash
python -m app.generators.midi.lyric_pipeline --production-dir <dir> --song-proposal <yml>
```
...
```bash
python -m app.generators.midi.drift_report --production-dir <dir>
```
...
```bash
python -m app.generators.midi.song_evaluator <production-dir>

# With ACE import metrics:
python -m app.generators.midi.song_evaluator <production-dir> --ace-import

# Re-score chromatic alignment against actual color target:
python -m app.generators.midi.song_evaluator <production-dir> --rescore

# Re-score lyrics post-ACE edit:
python -m app.generators.midi.song_evaluator <production-dir> --rescore-lyrics
```
...
python -m app.generators.midi.song_evaluator <dir> --rescore-lyrics

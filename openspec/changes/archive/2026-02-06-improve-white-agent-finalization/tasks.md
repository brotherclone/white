...
- [x] **2.1** Modify meta-rebracketing prompt to only list agents present in `transformation_traces`
- [x] **2.2** Include each agent's `document_synthesis` in the aggregation
- [x] **2.3** Replace hardcoded boundary descriptions with actual `boundaries_shifted` from traces
- [x] **2.4** Add agent-specific artifact counts to context

## Phase 3: Enrich Transformation Traces

- [x] **3.1** Add `content_excerpt: Optional[str]` field to `TransformationTrace`
- [x] **3.2** Add `artifact_count: int` field to `TransformationTrace`
- [x] **3.3** Update each `process_*_agent_work` method to populate new fields
- [x] **3.4** Update `_format_transformation_traces` to include excerpts

## Phase 4: Synthesis Prompt Variation

- [x] **4.1** Create `SynthesisPromptTemplate` enum or similar structure
- [x] **4.2** Implement 3-4 distinct synthesis templates (temporal, ontological, emotional, structural)
- [x] **4.3** Add logic to select template based on dominant patterns in traces
- [x] **4.4** Test each template produces meaningfully different output

## Phase 5: Per-Agent Synthesis Sections

- [x] **5.1** Create `_generate_agent_mini_synthesis(agent_name, traces, artifacts)` method
- [x] **5.2** Call for each agent that ran, preserving their unique lens
- [x] **5.3** Aggregate mini-syntheses with clear section headers
- [x] **5.4** Ensure final document maintains coherence while preserving diversity

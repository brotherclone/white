...
- [x] 1.1 Implement `segment_ace_export_by_arrangement(word_events, arrangement_path, bpm) -> dict[str, list]`
...
## 2. Drift Report
- [x] 2.1 Implement `compare_section(approved_midi_path, ace_word_events) -> dict` — for one section:
...
- [x] 2.2 Implement `generate_drift_report(production_dir) -> dict` — calls `load_ace_export`,
        segments by arrangement, compares each section, aggregates `overall_pitch_match`,
        `overall_rhythm_drift`, `total_lyric_edits`
- [x] 2.3 Write `drift_report.yml` to the production directory with per-section and overall metrics
- [x] 2.4 CLI: `python -m app.generators.midi.drift_report --production-dir <dir>`

## 3. Song Evaluator Extension
- [x] 3.1 Add `--ace-import` flag to `song_evaluator.py`
- [x] 3.2 When flag is set and `load_ace_export` returns data:
...
- [x] 3.3 Append `actual_vocal_coverage`, `actual_syllable_density`, `ace_chromatic_alignment` to
        `song_evaluation.yml` under a new `ace_actuals` key

## 4. Tests
- [x] 4.1 Unit: `segment_ace_export_by_arrangement` — words before first section omitted, words in gap
        between sections omitted, words straddling a boundary assigned to the section they start in
- [x] 4.2 Unit: `compare_section` — identical section returns 100% pitch match, 0 rhythm drift,
        edit_distance=0; transposed section returns reduced pitch match
- [x] 4.3 Unit: `generate_drift_report` with mocked `load_ace_export` and arrangement
- [x] 4.4 Integration: run drift report against `blue__rust_signal_memorial_v1` and assert

...
- [x] 1. Add post-workflow shrinkwrap to `start_workflow()` in `app/agents/white_agent.py`:
...
- [x] 2. Add `POST /generate` endpoint to `app/tools/candidate_server.py`:
...
- [x] 3. Add `GET /generate/status` endpoint to `app/tools/candidate_server.py`:
...
- [x] 4. Add `startGenerate()` and `getGenerateStatus()` fetch helpers to `web/lib/api.ts`

- [x] 5. Add Generate button to `web/app/page.tsx`:
...
- [x] 6. Add tests for the two new endpoints:

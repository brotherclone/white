...
- [x] 1.1 Implement `build_loop_inventory(production_dir) -> dict` — reads approved
...
- [x] 1.2 Implement `load_song_proposal(production_dir) -> dict` — locates the song
...
- [x] 2.1 Implement `build_prompt(proposal, inventory) -> str` — constructs the
...
- [x] 2.2 Implement `call_claude(prompt) -> str` — calls `claude-sonnet-4-6` via
        Anthropic SDK, returns the raw text response. Raises on API error.
- [x] 2.3 Implement `parse_response(raw) -> dict` — extracts structured proposal from
...
## 3. Output
- [x] 3.1 Implement `write_proposal(production_dir, proposal_dict)` — writes
...
- [x] 3.2 CLI: `python -m app.generators.midi.composition_proposal --production-dir <dir>`
        Exits with a clear message if no approved loops found. Warns but proceeds if
        only some instruments have approved loops.

## 4. Drift Integration
- [x] 4.1 Extend `compute_drift` in `assembly_manifest.py` to accept an optional
...
- [x] 4.2 Write `proposal_drift` block into the existing drift report output when
        a proposal is present.

## 5. Tests
- [x] 5.1 Unit: `build_loop_inventory` with mocked review files — correct labels,
        bar counts, energy scores extracted
- [x] 5.2 Unit: `parse_response` — valid fenced YAML extracted; graceful fallback
        on malformed response
- [x] 5.3 Unit: `compute_drift` with proposal_path — sections_added, sections_removed,
        repeat_deltas computed correctly
- [x] 5.4 Integration: run against `blue__rust_signal_memorial_v1` with mocked Claude

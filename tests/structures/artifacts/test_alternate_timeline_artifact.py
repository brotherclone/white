...
def test_narrative_too_short_raises():
    """Test that narratives under 100 words raise ValidationError."""
    divergence = DivergencePoint(
        when="After graduating college in 1997",
        what_changed="Took the Greyhound to Portland",
        why_plausible="Had been offered a job",
    ...
    # Create a narrative with less than 100 words (this is about 20 words)
    short_narrative = "This is a very short narrative that does not meet the minimum requirement of one hundred words for validation."
    ...
    assert "Narrative too short" in str(exc_info.value)


def test_narrative_exactly_100_words_passes():
    """Test that narratives with exactly 100 words pass validation."""
    divergence = DivergencePoint(
        when="After graduating college in 1997",
        what_changed="Took the Greyhound to Portland",
        why_plausible="Had been offered a job",
    ...
    # Create a narrative with exactly 100 words
    exact_100_words = " ".join(["word"] * 100)

    m = AlternateTimelineArtifact(
        period=period,
        title="Test",
        narrative=exact_100_words,
        divergence_point=divergence,
        key_differences=["test"],
        specific_details=create_valid_details(),
        emotional_tone=QuantumTapeEmotionalTone.MELANCHOLY,
    ...
    assert isinstance(m, AlternateTimelineArtifact)
    assert len(m.narrative.split()) == 100


def test_insufficient_details_raises():
    """Test that fewer than 5 specific details raise ValidationError."""
    divergence = DivergencePoint(
        when="After graduating college in 1997",
        what_changed="Took the Greyhound to Portland",
        why_plausible="Had been offered a job",
    ...
    # Create only 3 details (less than required 5)
    insufficient_details = [
        AlternateLifeDetail(
            category="career",
            detail="Working the poetry section",
            sensory_elements=["smell of books"],
        ),
        AlternateLifeDetail(
            category="location",
            detail="Renting an apartment",
            sensory_elements=["foggy mornings"],
        ),
        AlternateLifeDetail(
            category="creative",
            detail="Attending poetry readings",
            sensory_elements=["espresso aroma"],
        ),
    ]
    ...
    with pytest.raises(ValidationError) as exc_info:
        AlternateTimelineArtifact(
            ...
            divergence_point=divergence,
            key_differences=["test"],
            specific_details=insufficient_details,
            emotional_tone=QuantumTapeEmotionalTone.MELANCHOLY,
            mood_description="test",
            preceding_events=[],
            following_events=[],
    ...
    assert "Not enough specific details" in str(exc_info.value)
    assert "3/5" in str(exc_info.value)


def test_exactly_5_details_passes():
    """Test that exactly 5 specific details pass validation."""
    divergence = DivergencePoint(
        when="After graduating college in 1997",
        what_changed="Took the Greyhound to Portland",
        why_plausible="Had been offered a job",
    ...
    m = AlternateTimelineArtifact(
        ...
        divergence_point=divergence,
        key_differences=["test"],
        specific_details=create_valid_details(),  # This creates exactly 5 details
        emotional_tone=QuantumTapeEmotionalTone.MELANCHOLY,
        mood_description="test",
        preceding_events=[],
        following_events=[],
